...
本地家庭财务 Web 应用，支持支付宝/微信支出导入、截图资产识别、ECharts 可视化仪表盘、Excel 标准化输出。
...
│   │   └── ocr_service.py      # OCR 识别（已降级为手动输入模式）
│   ├── scripts/
│   │   ├── migrate_history.py  # 历史数据迁移（xlsx → JSON 缓存）
│   │   └── parse_month_folder.py # 解析月份文件夹（xlsx + 截图）
│   └── requirements.txt
├── frontend/
│   ├── src/
│   │   ├── App.tsx             # 路由配置
...
└── <bill_dir>/                   # 源数据目录（xlsx + 截图，由 config.py 配置）
    ├── 2022/ ~ 2026/            # 历史月份文件夹
    └── currMonth/                # 当月待处理数据
```
...
1. 将 xlsx + 截图整理到 `{bill_dir}/<月份>/` 目录
2. 在 Claude Code 中执行：`/import-month <月份>`
3. 自动完成：
   - 解析 xlsx 提取支出/收入
   - Tesseract OCR 提取截图中的资产余额
   - 调用 API 写入系统
...
| POST | `/api/ocr/confirm` | 确认资产/收入数据 |
