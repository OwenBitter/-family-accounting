---
name: import-month
description: 从文件夹导入月度数据。解析 xlsx 提取支出/收入，OCR 截图提取资产数据，调用 Flask API 写入系统。当用户请求"导入X月数据"或提到"整理账单"时自动触发。
arguments:
...
1. 解析 xlsx 文件 → 提取支出/收入数据
2. Tesseract OCR 分析截图 → 提取资产余额
3. 调用 Flask API → 写入 data/history/ + Excel
...
```bash
cd e:/tools/family-accounting
TESSDATA_PREFIX=~/tessdata python backend/scripts/ocr_analyze.py "<folder>"
```

读取 OCR 结果中的资产数据（余额宝、零钱、基金等）。
...
**4b. 导入资产/收入：**
```bash
curl -X POST http://localhost:5000/api/ocr/confirm \
  -H "Content-Type: application/json" \
  -d '{"month":"<month>","person":"BB","data":{...},"income_records":[...]}'
```
...
- OCR 依赖 Tesseract 5.4，路径：`C:\Program Files\Tesseract-OCR\tesseract.exe`
- 中文语言包：`~/tessdata/chi_sim.traineddata`
- 必须设置环境变量 `TESSDATA_PREFIX=~/tessdata`
...
3. 运行 `ocr_analyze.py` 分析截图
4. 调用 API 导入数据
5. 告知用户查看 http://localhost:5173
